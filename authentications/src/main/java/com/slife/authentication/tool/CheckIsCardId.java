...
public class CheckIsCardId {
     ...
     * @param tip   	显示的证件名字（驾驶证号,身份证号）
     */
    public CheckIsCardId(String idCardNum,String tip) {
        // super();
        this.idCardNum = idCardNum.trim().toUpperCase();
        errMsg=new String[] {tip+"完全正确！",
                tip+"不能为空！",
                tip+"长度为15或18位数！",
                tip+"有非法字符！",
                tip+"中出生日期不合法！",
    ...
    public int isCorrect(){
        if(this.isEmpty()){
            this.error=CheckIsCardId.IS_EMPTY;
            return this.error;
        }

        if(!this.isLenCorrect()){
            this.error=CheckIsCardId.LEN_ERROR;
            return this.error;
        }

        if(!this.isCharCorrect()){
            this.error=CheckIsCardId.CHAR_ERROR;
            return this.error;
        }

        if(!this.isDateCorrect()){
            this.error=CheckIsCardId.DATE_ERROR;
            return this.error;
        }

        if(this.is18()){
            if(!this.getCheck().equals(this.getCheckBit())){
                this.error=CheckIsCardId.CHECK_BIT_ERROR;
                return this.error;
            }
        }
